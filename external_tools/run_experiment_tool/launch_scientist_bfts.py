...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-5",
        help="Model to use for plot aggregation (gpt-5, gpt-5, gpt-5-mini, gpt-5-nano)",
    )
    parser.add_argument(
        "--model_writeup",
        type=str,
        default="gpt-5",
        help="Model to use for writeup (gpt-5, gpt-5, gpt-5-mini, gpt-5-nano)",
    )
    parser.add_argument(
        "--model_citation",
        type=str,
        default="gpt-5",
        help="Model to use for citation gathering (gpt-5, gpt-5, gpt-5-mini, gpt-5-nano)",
    )
    parser.add_argument(
        "--num_cite_rounds",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-5",
        help="Model to use for review main text and captions (gpt-5, gpt-5, gpt-5-mini, gpt-5-nano)",
    )
    parser.add_argument(
        "--skip_writeup",
        action="store_true",
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="If set, preserve all temporary files and directories (no cleanup)",
    )
    parser.add_argument(
        "--end_stage",
        type=int,
        default=4,
        choices=[1, 2, 3, 4], 
        help="Final stage to run (1=initial_implementation, 2=baseline_tuning, 3=creative_research, 4=ablation_studies)"
    )
...
if __name__ == "__main__":
    ...
    idea_config_path = edit_bfts_config_file(
        ...
        idea_dir,
        idea_path_json,
        end_stage=args.end_stage,  # ADD THIS LINE
    )

    perform_experiments_bfts(idea_config_path, debug=args.debug)
    ...
    # Only cleanup experiment_results if not in debug mode
    if not args.debug:
        experiment_results_path = osp.join(idea_dir, "experiment_results")
        if os.path.exists(experiment_results_path):
            print("Cleaning up experiment_results directory...")
            shutil.rmtree(experiment_results_path)
    else:
        print("Debug mode: preserving experiment_results directory")
    ...
    # BUT ONLY among child processes, not all system processes
    # This prevents killing parent processes like launch_multiagent.py
    keywords = ["torch", "mp", "bfts", "experiment"]  # Removed "python" to avoid killing parent processes
    
    # Only check child processes and their descendants, not all system processes
    all_child_pids = set()
    for child in children:
        try:
            all_child_pids.add(child.pid)
            # Also add grandchildren and deeper descendants
            for descendant in child.children(recursive=True):
                all_child_pids.add(descendant.pid)
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
    
    # Only cleanup processes that are actually descendants of this AI-Scientist process
    for pid in all_child_pids:
        try:
